...
async function tryFetch(url){
  try{
    ...
    console.log(`Probando ${url} -> ${JSON.stringify(r.data).slice(0,150)}`);
    ...
    return null;
  }catch(e){ console.log(`Falló ${url}: ${e.message}`); return null; }
}
...
async function getStock(){
  ...
  for(const u of urls){
    const s = await tryFetch(u);
    if(s) return s;
  }
  console.log('Todas las APIs vacías, devolviendo null');
  return null;
}
...
app.get('/',(req,res)=>res.send('BOT ON - API con error Scraping failed es normal ahora'));
