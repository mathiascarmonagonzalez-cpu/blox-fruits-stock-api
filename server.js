...
async function getStock(){
  ...
  const apis = [
    ...
    'https://api.fruityblox.com/stock',
    'https://blox-fruits.com/api/stock',
    'https://fruityblox.vercel.app/api/stock'
  ];
  for(let api of apis){
    try{
      const {data} = await axios.get(api,{timeout:8000,headers:{'User-Agent':'Mozilla/5.0'}});
      let normal = data.normal||data.Normal||data.data?.normal||data.stock||[];
      ...
      if(normal.length>0) return {normal,mirage,source:api};
    }catch(e){}
  }
  // 2. Scraping con proxy que SI funciona en Render
  try{
    const url = `https://api.allorigins.win/raw?url=${encodeURIComponent('https://fruityblox.com/stock')}`;
    const {data} = await axios.get(url,{timeout:12000});
    const $ = cheerio.load(data);
    let fruits = [];
    $('*').each((i,el)=>{
      const t = $(el).text().trim().split('\n')[0].trim();
      if(BELI[t] &&!fruits.includes(t) && t.length<20) fruits.push(t);
    });
    if(fruits.length>0) return {normal:fruits.slice(0,6),mirage:fruits.slice(6,10),source:'scrape'};
  }catch(e){}
  return null;
}
...
async function sendDiscord(data){
  ...
  const format = (n,t)=>`${EMOJI[n]||'🍈'} ${n} ($${(BELI[n]||0).toLocaleString()} - ${ROBUX[n]||'?'} Robux) [${t}]`;
  const imp = [...data.normal,...data.mirage].filter(f=>IMPORTANT.includes(f));
  if(imp.length>0){
    for(let f of imp){
      const type = data.normal.includes(f)?'Normal':'Mirage';
      await ch.send({content:`@everyone ${EMOJI[f]||'🔥'} **${f} en stock** ($${BELI[f].toLocaleString()} - ${ROBUX[f]} Robux) [${type}]`});
    }
  }
  const embed = new EmbedBuilder().setTitle(imp.length?'🚨 STOCK IMPORTANTE':'🍈 Stock Actual').setColor(imp.length?0xFF0000:0x00FF00).setTimestamp().setDescription(`__**☀️ Normal:**__\n${data.normal.map(n=>`• ${format(n,'Normal')}`).join('\n')}\n\n__**🌙 Mirage:**__\n${data.mirage.length?data.mirage.map(n=>`• ${format(n,'Mirage')}`).join('\n'):'Vacío'}\n\nFuente: ${data.source||'api'}`);
  await ch.send({embeds:[embed]});
}

function startLoop(){
  setInterval(async()=>{const s=await getStock(); if(s) await sendDiscord(s);},4*60*60*1000+60000);
...
app.get('/test',async(req,res)=>{
  ...
  if(s){await sendDiscord(s); res.send(`Test OK ${s.source}: ${JSON.stringify(s)}`);}
  else res.send('No stock - APIs caidas, intenta en 2 min');
});
